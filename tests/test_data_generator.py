...
def test_rto_rate_realistic(df):
    rto_rate = float(df["is_rto"].mean())
    assert 0.10 <= rto_rate <= 0.40, f"Unexpected RTO rate: {rto_rate:.3f}"


def test_no_null_values(df):
    assert int(df.isnull().sum().sum()) == 0, "Unexpected nulls in dataset"


def test_timestamps_sorted(df):
    ts = df["order_timestamp"].values
    assert bool((ts[1:] >= ts[:-1]).all()), "Timestamps not sorted ascending"


def test_cod_higher_rto(df):
    """COD orders should have higher average RTO rate than prepaid."""
    cod_rto = float(df[df["is_cod"] == 1]["is_rto"].mean())
    prep_rto = float(df[df["is_cod"] == 0]["is_rto"].mean())
    assert cod_rto > prep_rto, f"COD RTO ({cod_rto:.3f}) not > prepaid ({prep_rto:.3f})"


def test_feature_ranges(df):
    assert bool((df["address_completeness_score"].between(20, 100)).all())
    assert bool((df["pincode_rto_rate"].between(0.02, 0.65)).all())
    assert bool((df["order_value"].between(99, 49_999)).all())
    assert bool((df["is_cod"].isin([0, 1])).all())
    assert bool((df["is_rto"].isin([0, 1])).all())
...
def test_customer_rto_rate_bounds(df):
    assert bool((df["customer_rto_rate"] >= 0).all())
    assert bool((df["customer_rto_rate"] <= 1).all())
